Function Get-ZPAConnectors{
    ...
    .EXAMPLE
        $appconnectors = Get-ZPAConnectors -token $token -customerid $customerid -zscalercloud config.zpagov.net
    ...
    $apiurl = "https://$($zscalercloud)/mgmtconfig/v1/admin/customers/$($customerid)/connector"
