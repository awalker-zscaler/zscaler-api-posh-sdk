Function Get-ZPAIssuedCertificates{
    ...
    .EXAMPLE
        $issuedcertificates = Get-ZPAIssuedCertificates -token $token -customerid $customerid -zscalercloud config.zpagov.net
    ...
    $apiurl = "https://$($zscalercloud)/mgmtconfig/v1/admin/customers/$($customerid)/certificate/issued"
