Function Get-ZPACertificates{
    ...
    .EXAMPLE
        $certificates = Get-ZPACertificates -token $token -customerid $customerid -zscalercloud config.zpagov.net
    ...
    $apiurl = "https://$($zscalercloud)/mgmtconfig/v1/admin/customers/$($customerid)/certificate"
